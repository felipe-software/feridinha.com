...
const AlbumItem = ({ upload }: { upload: Upload }) => {
    ...
    return (
        <div className="item" key={upload.name}>
            <div className="title">
                <a
            ...
            {type === "invalid" && (
                <div className="preview-wrapper">
                    <p>Preview não disponível. Acesse o link acima.</p>
                </div>
            )}
        </div>
    )
...
export const AlbumPage = () => {
    ...
    const album = useAlbumQuery(Array.isArray(albumId) ? albumId[0] : albumId)

    const uploads = album.data?.uploads
                    ...
                    <div className="header">
                        <p>
                            ...
                            <a
                                ...
                                href="#"
                                target="_blank"
                                style={{ color: album.data.user.color }}
                            >
                                @{album.data.user.name}
                            </a>{" "}
                            {dayjs(album.data.createdAt).fromNow()}.{" "}
                            {uploads!.length} arquivo
                            {uploads!.length !== 1 ? "s" : ""} no total.{" "}
                            {album.data.viewCount} visualizações.
                        </p>
                    </div>
                    <div className="items">
                        {album.data.uploads.map((upload) => (
