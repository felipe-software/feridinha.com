ALTER TABLE "OAuthAccount" ADD COLUMN "profileImage" TEXT;

-- User.profileImage came from the provider that originally created the user.
-- Backfill it only when that original identity is unambiguous.
UPDATE "OAuthAccount" AS account
SET "profileImage" = users."profileImage"
FROM "User" AS users
