...
import { extFromContentType, uploadExternalPost } from "@/services/external-post/uploader";
...
describe("safeFetchExternal", () => {
    ...
    test("rejeita credenciais e hosts fora das duas allowlists", async () => {
        const controller = new AbortController();
        await expect(
            validateExternalDestination(
        ...
        ).rejects.toEqual(expect.objectContaining({ code: "host_not_allowed" }));
    });
        ...
        try {
            await expect(
                safeFetchExternal(`http://localhost:${server.port}/redirect`, {
                    ...
                    globalHosts: ["localhost", "127.0.0.1"],
                    maxBytes: 100,
